...

{
    ...
    kernel_parameter_chk() {
        l_krp="$(sysctl "$l_kpname" | awk -F= '{print $2}' | xargs)" # Aktuelle Konfiguration überprüfen
        ...
        unset A_out
        declare -A A_out # Überprüfen der dauerhaften Einstellungen (Dateien)
        ...
        if [ -n "$l_ufwscf" ]; then # UFW-Systeme berücksichtigen

...

{
    ...
    if dpkg-query -s cron &>/dev/null; then
        # Überprüfen der Berechtigungen, Uid und Gid von /etc/cron.monthly/
        ...
        if [[ "$stat_output" == "Access: (700/drwx------) Uid: ( 0/ root) Gid: ( 0/ root)" ]]; then
            l_output=" - Berechtigungen und Benutzer für /etc/cron.monthly/ sind korrekt."
        else
            l_output2=" - Berechtigungen oder Benutzer für /etc/cron.monthly/ sind nicht korrekt:\n   $stat_output"
        fi
    else
